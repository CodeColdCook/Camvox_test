...
{
  /********************************************Calibration**************************************************************************/
  Calibrating::Calibrating(string _strSettingPath) : mbResetRequested(false), mbFinishRequested(false), mbFinished(true),
   mbStopped(false), mbStopRequested(false), mbNotStop(false), mbCalibrating(false), mbOptimizing(false),cumulative_flag(false)
  {
    ...
    RGBPath = "/home/cbreezy/002_everything_here/104-CamVox-test/Camvox_calibr_test1/calibration/calibration.bmp";
    PcdPath = "/home/cbreezy/002_everything_here/104-CamVox-test/Camvox_calibr_test1/calibration/calibration.pcd";
    projectionType = "depth";
    color2dDepth = false;
    isEnhanceImg = false;
    isFillImg = true;
  }
  ...
  void Calibrating::initialize(string _RGB_path, string _Pcd_path, string _Projection_type, bool _isEnhanceImg, bool _isFillImg)
  {
    ...
    rotation_matrix_adjust = Eigen::AngleAxisd(euler_angle[0], Eigen::Vector3d::UnitZ()) * Eigen::AngleAxisd(euler_angle[1],
                                                                    Eigen::Vector3d::UnitY()) * Eigen::AngleAxisd(euler_angle[2],
...
#ifdef debug
    color3d(extrinsic_params, 1, "/home/cbreezy/002_everything_here/104-CamVox-test/Camvox_calibr_test1/debug/test.pcdl");  
  ...
  void Calibrating::Run()
  {
    ...
    // while (1)
    cout << "mbCalibrating: " << mbCalibrating << endl;
    cout << "cumulative_flag: " << cumulative_flag << endl;
    ...
    cout << "costFunction result: " << init_distance << endl;
  ...
  /***************************************************extractEdge*******************************************************************/
  void Calibrating::extractEdge(const bool is_filter, const Mat &img, const vector<vector<Point>> &contous, 
                                  const int min_len, Mat &edge_img, pcl::PointCloud<pcl::PointXYZ>::Ptr &edge_cloud)
  {
  ...
  float Calibrating::costFunction(const double *abc)
  {
    ...
    adjust_rotation_matrix = Eigen::AngleAxisd(eluer[0], Eigen::Vector3d::UnitZ()) * Eigen::AngleAxisd(eluer[1], 
                                Eigen::Vector3d::UnitY()) * Eigen::AngleAxisd(eluer[2], Eigen::Vector3d::UnitX());
  ...
  void Calibrating::color2d(const bool use_depth, const string info)
  {
    ...
    if (use_depth)
    {
      if(imdepth_true_.empty()){
        cout << "imdepth_true_ is empty, please check something!!!" << endl;
        return;
      }
      Mat imdepth8_true;
    ...
    {
      if(imIntensity8_.empty()){
        cout << "imIntensity8_ is empty, please check something!!!" << endl;
        return;
      }
...
// cv::waitKey();
  ...
  bool Calibrating::initRgbUseClouds()
  {
    ...
    rgb_use_cloud_->width = cloud1->points.size(); // cloud1只进行了初始化，并没有数据
